...
//
// v0.13: 2013-03-04
//      Support for Arduino 1.5 (SPI.h included, additional changes for DUE support)format
//
// v0.12: 2013-03-01
...
// This enables the Robot commands
#define ROBOT

// This enables LOAD, SAVE, FILES command through the RamDisk Library
#define ENABLE_RAMDISK 1
//#undef ENABLE_RAMDISK

#define RAMDISK_BLOCKS 128
...
// This makes the RamDisk Library use an external EEPROM
#define USE_EEPROM_FOR_DISK 1

// This makes tje RamDisk Library use an external SRAM
//#define USE_SRAM_FOR_DISK 1
#undef USE_SRAM_FOR_DISK 1
...
#undef USE_SRAM_FOR_DISK
#ifdef USE_SRAM_FOR_DISK
#include <M23LCV512.h>
#define RD_CS 10
M23LCV512 ram;
#endif
...
#if defined(ENABLE_FILEIO) || defined(ENABLE_RAMDISK)
...
enum {
    KW_LIST = 0,
    KW_LOAD, KW_NEW, KW_RUN, KW_SAVE, KW_DEL,
...
static const unsigned char rdremovemsg[]	  PROGMEM = "Failed.";
...
#ifdef ROBOT
static unsigned char write_quoted_string(void) {
    int i = 0;
    unsigned char delim = *txtpos;
    if (delim != '"' && delim != '\'') {
        return 0;
...
boolean get2s(short int &first, short int &second);
boolean get2s(short int &first, short int &second) {
    expression_error = 0;
    first = expression();
    if (expression_error) {
...
pendown:
    ...
    // MOVE x,y : Moves in a straight line to point x,y without drawing
    // (regardless of the current pen status)
...
move:
    ...
    // RMOVE x,y : Moves in a straight line to a point x units away along the x-axis and y units away along the y-axis, without drawing
    // (regardless of the current pen status)
...
rmove:
    ...
    bot->moveTo(x + bot->getX(), y + bot->getY(), 1);
    goto execnextline;

    // DRAW x,y : Moves in a straight line to a point x units away along the x-axis and y units away along the y-axis, drawing as it goes
    // (regardless of the current pen status)
...
draw:
    ...
    // RDRAW x,y : Moves in a straight line to a point x units away along the x-axis and y units away along the y-axis, drawing as it goes
    // (regardless of the current pen status)
...
rdraw:
    ...
    bot->moveTo(x + bot->getX(), y + bot->getY(), 0);
...
dir:
    ...
    // FACE x,y : Turn to face the point x units away alongthe x-axis and y units away along the y-axis.
...
face:
    ...
    // CIRCLE radius : Draws a circle of the given radius, centred on the current location.
...
circle:
    ...
    // ELLIPSE xradius,yradius: Draws an ellipse of the given Xradius and Yradius, centred on the current location.
...
ellipse:
    ...
    // WRITE expression : Writes the value of the expression at the current location
...
format:
#ifdef ENABLE_RAMDISK
    if (vol.format(&ram, RAMDISK_BLOCKS, 16, 1)) {
        printmsg(okmsg);
    } else {
        printmsg(rdremovemsg);
    }
...
fonts:
#ifdef EEEPROM_FONTS 
#define EEEPROM_FONTS_TABLE 0x10000
	{
		char buffer[32];
		boolean again = true;
		byte font = 0;
		...
		while (again) {
			ram.read(EEEPROM_FONTS_TABLE+(font * 256), buffer, 32);
			if (buffer[0] == 'F' && buffer[1] == 'O' && buffer[2] == 'N' && buffer[3] == 'T') {
				Serial.print(F("Font "));
				Serial.print(font);
				Serial.print(F(": "));
...
del:
...
#ifdef ENABLE_RAMDISK
    {
        ...
        // Work out the filename
        expression_error = 0;
        filename = (unsigned char *) filenameWord();
        if (expression_error) {
            goto qwhat;
        }

        if (!vol.remove((const char *)filename)) {
            printmsg(rdremovemsg);
        }
    }
    goto warmstart;
...
void setup() {
    ...
    ram.begin(10);
#endif
#ifdef USE_EEPROM_FOR_DISK
    ram.begin();
#endif
    if (!vol.init(&ram)) {
        Serial.print(F("RamDisk init fail"));
    }
#endif
...
// if this is a robot check the state of pin A1; 
// if A1 is high Autorun from the builtin eeprom 
...
static int initSD(void) {
    ...
    // an output, even when your shield uses another line for CSelist
...
void cmd_Files(void) {
    ...
    vol.ls(&Serial, LS_SIZE);
    Serial.print((long) vol.freeClusterCount()* vol.clusterSizeBytes());
